...
import { FaCheckCircle, FaComments, FaPaperPlane, FaTimes } from 'react-icons/fa';
import { isCurrentUsersMessage } from '../utils/chatIdentity';
import './GuideBookingChatModal.css';
...
function ChatMessageBubble({ message, currentUserId, showSender, startsGroup, endsGroup }) {
...
export default function GuideBookingChatModal({
    isOpen,
    onClose,
    booking,
    thread,
    ...
    const headerName = thread?.counterpart_name || booking.traveler_name || 'Booking Chat';
    const headerAvatar = thread?.counterpart_avatar || booking.avatar || '';
    const headerMeta = `${booking.destination} · ${booking.trip_start} – ${booking.trip_end}`;
                    ...
                    {loading ? (
                        ...
                        <div className="chat-message-list">
                            {thread.messages.map((message, index) => {
                                ...
                                const previousOwn = previous ? isOwnMessage(previous, currentUserId) : null;
                                const currentOwn = isOwnMessage(message, currentUserId);
                                const nextOwn = next ? isOwnMessage(next, currentUserId) : null;
